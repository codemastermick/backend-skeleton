...
export default class AuthRoutes extends CommonRoutesConfig {
    ...
    configureRoutes(): express.Application {
        this.app.route("/register").post(async (req: express.Request, res: express.Response) => {
        ...
        this.app.route("/login").post(async (req: express.Request, res: express.Response) => {
            ...
            const user = await this.dbService.readOne<User>("User", userSchema, { username, password });
        ...
        this.app.post('/token', (req: express.Request, res: express.Response) => {
            ...
            jwt.verify(token, process.env.JWT_REFRESH_SECRET, (err: Error, user: JwtPayload) => {
                if (err) {
                    return res.sendStatus(StatusCodes.FORBIDDEN);
                }

                const accessToken = jwt.sign({ username: user.username as string, role: user.role as string }, process.env.JWT_SIGNING_SECRET, { expiresIn: '20m' });
        ...
        this.app.route("/logout").post((req: express.Request, res: express.Response) => {
            if (!authenticationEnabled) {
                throw new ApplicationException(StatusCodes.NOT_IMPLEMENTED, "Authentication is not enabled for this project. Please see src/configs/features.config.ts to enable a database.");
            }
