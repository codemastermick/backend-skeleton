...
export default class Server {
  ...
  constructor() {
    this.logger = new Logger(
      process.env.APP_NAME.toUpperCase().replace(/ /g, '-')
    );
    this.app = express();
    this.setPort();
  ...
  private configureRoutes() {
    ...
    this.routes.push(new CrudRouter(this.app));
  }
  ...
  private setPort() {
    ...
    if (this.validatePortNumber(port)) {
      ...
      this.logger.debug(
        `Port [${port}] is not usable! Please set the port to between 1001 and 65535. Continuing on port 3000`
      );
      this.port = 3000;
    }
  }
